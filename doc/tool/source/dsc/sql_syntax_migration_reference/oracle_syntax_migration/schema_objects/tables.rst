...
SEGMENT CREATION { IMMEDIATE \| DEFERRED } is not supported in GaussDB(DWS), hence it is commented in the migrated output. This is based on the following configuration item: **commentStorageParameter=true**.
...
Storage parameters including **BUFFER_POOL** and **MAXEXTENTS** are not supported in GaussDB(DWS). Storage parameters are commented when it appears in tables or indexes based on the value of the config parameter **comment_storage_parameter**.
...
If the configuration parameter :ref:`foreignKeyHandler <en-us_topic_0000001772536388__en-us_topic_0000001658024910_en-us_topic_0218440495_li19969157459>` is set to **true** (default value), then the tool will migrate these statements into commented statements.
