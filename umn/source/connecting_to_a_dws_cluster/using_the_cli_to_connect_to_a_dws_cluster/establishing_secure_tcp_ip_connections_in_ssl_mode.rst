...
.. _en-us_topic_0000002235494444__section131774823014:
...
#. In the cluster list, click a cluster name. On the page displayed, click the **Security Settings** tab.
...
#. In the **SSL Connection** area, enable **Require SSL Connection** (recommended).
   ...
   -  Enabled: Set **require_ssl** to **1**, indicating that the server forcibly requires SSL connections.
   ...
   -  Disabled: Set **require_ssl** to **0**, indicating that the server does not forcibly require SSL connections. By default, this function is disabled. For details about how to configure the **require_ssl** parameter, see :ref:`require_ssl (Server) <en-us_topic_0000002235494444__li107621516191913>`.
...
.. _en-us_topic_0000002235494444__s6d3b0bb119894929810147678d9c67a5:
...
#. .. _en-us_topic_0000002235494444__li13478842115911:
...
#. Run the export command and configure digital certificate parameters related to SSL authentication on the host where the gsql client is installed.
   ...
   There are two SSL authentication modes: bidirectional authentication and unidirectional authentication. The client environment variables to be configured vary according to the authentication mode and must contain the absolute file paths. For details, see :ref:`SSL Authentication Modes and Client Parameters <en-us_topic_0000002235494444__s3a228fb4ac9c48ec8bc34e812c8879e8>`.

   Two-way authentication is recommended. The following parameters are required:
...
.. _en-us_topic_0000002235494444__s3a228fb4ac9c48ec8bc34e812c8879e8:

SSL Authentication Modes and Client Parameters
----------------------------------------------

There are two SSL authentication modes: bidirectional authentication and unidirectional authentication. Table :ref:`Table 1 <en-us_topic_0000002235494444__table267519441727>` shows the differences between these two modes. You are advised to use bidirectional authentication for security purposes.

.. _en-us_topic_0000002235494444__table267519441727:
...
Configure environment variables related to SSL authentication on the client. For details, see :ref:`Table 2 <en-us_topic_0000002235494444__t8b0644779e4c40009b6fb1ad6a8ea986>`. The path of environment variables is set to **/home/dbadmin/dws_ssl/** as an example. Replace it with the actual path.

.. _en-us_topic_0000002235494444__t8b0644779e4c40009b6fb1ad6a8ea986:
...
.. table:: **Table 2** Client parameters
   ...
   |                       |                                                                                                                                                                                             | -  **verify-full**: DWS does not support this mode.                                                                                                                                                    |
   |                       |                                                                                                                                                                                             |                                                                                                                                                                                                        |
   |                       |                                                                                                                                                                                             | Default value: **prefer**                                                                                                                                                                              |
   |                       |                                                                                                                                                                                             |                                                                                                                                                                                                        |
   |                       |                                                                                                                                                                                             | **Note:** If an external client accesses a cluster, the error message "ssl SYSCALL error" is displayed on some nodes. In this case, run **export PGSSLMODE="allow"** or **export PGSSLMODE="prefer"**. |
   +-----------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
.. _en-us_topic_0000002235494444__section1916311515557:

Combinations of SSL Connection Parameters on the Client and Server
------------------------------------------------------------------

Whether the client uses the SSL encryption connection mode and whether to verify the server certificate depend on client parameter **sslmode** and server (DWS cluster) parameters **ssl** and **require_ssl**. The parameters are as follows:
...
-  **ssl (Server)**
   ...
   -  The default value is **on** for clusters whose version is 1.3.1 or later, and you cannot set this parameter on the DWS console.
   -  For clusters whose version is earlier than 1.3.1, the default value is **on**. You can set the **require_ssl** parameter in the **Require SSL Connection** area of the cluster's **Security Settings** page on the DWS console.

-  .. _en-us_topic_0000002235494444__li107621516191913:
   ...
   **require_ssl (Server)**
   ...
   -  The default value is **off** for clusters whose version is 1.3.1 or later. You can set the **require_ssl** parameter in the **Require SSL Connection** area of the cluster's **Security Settings** page on the DWS console.
   -  For clusters whose version is earlier than 1.3.1, **the default value is** **off**, and you cannot set this parameter on the DWS console.
