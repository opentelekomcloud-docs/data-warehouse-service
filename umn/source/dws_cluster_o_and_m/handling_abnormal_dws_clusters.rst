...
A cluster in read-only status does not allow write operations. You can remove this status on the management console. Triggering the read-only status may be due to high disk usage, so you need to clean up the cluster data. For details, see "Solution to High Disk Usage and Cluster Read-Only" in Data Warehouse Service (DWS) Troubleshooting.
...
.. _en-us_topic_0000002235494480__section17567184318267:
