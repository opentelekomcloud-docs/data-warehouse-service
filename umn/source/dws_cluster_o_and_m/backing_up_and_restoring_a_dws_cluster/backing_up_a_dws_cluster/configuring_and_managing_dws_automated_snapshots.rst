...
.. figure:: /_static/images/en-us_image_0000002500730206.png
...
-  Fine-grained snapshots cannot back up comments and permissions of tables.
...
#. In the navigation pane, choose **Snapshots**. Click **Policies**. All policies (including the default automated snapshot policy) of the current cluster are displayed.

#. (Optional) Enable **Automated Snapshot**.
...
#. Click |image3| to set the retention mode and the backup media used by the current cluster for automated snapshots. For more information, see :ref:`Table 1 <en-us_topic_0000002329649414__en-us_topic_0000001423119273_en-us_topic_0000001360169333_en-us_topic_0000001231278872_table11860173413712>`.
...
#. Click **Add Snapshot Policy** and set the parameters. :ref:`Table 2 <en-us_topic_0000002329649414__en-us_topic_0000001423119273_en-us_topic_0000001360169333_en-us_topic_0000001231278872_table1355651818416>` describes the parameters.
   ...
   -  If you choose a full snapshot, **Snapshot Policy** can be set to **Periodic** or **One-off**. For schema-level full snapshots, you need to select the database associated with schemas. Up to 50 schemas can be selected at a time.
   -  If you choose an incremental snapshot, **Snapshot Policy** can only be **Periodic**. You can specify days in a week or dates, start time and interval.
   ...
   .. table:: **Table 2** Snapshot policy parameters
      ...
      |                                   |                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
      |                                   | -  For schema-level policies, the policy level, database, and schema attributes cannot be modified after configuration.                                                                                                                                                                                                                                                                                                                                           |
      +-----------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
      ...
      |                                   | -  **Time**: Specify the exact time on the selected days. For incremental snapshots, you can specify the start time and interval. The interval for cluster-level snapshots ranges from 4 to 24 hours, while for schema-level snapshots, it ranges from 1 to 24 hours. This setting determines the frequency at which snapshots will be taken. If the incremental data is large, the backup will be slow. In this case, increase the backup frequency.             |
      +-----------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
#. (Optional) Click the **Snapshots** tab. Click |image4| next to the target snapshot to view its policy name.

#. (Optional) To preview a policy, click **Preview Snapshot Policy**. The next seven snapshots of the cluster will be displayed. If no full snapshot policy is configured for the cluster, the default policy is used, that is, a full snapshot is taken after every 15 incremental snapshots.
...
You can copy automated snapshots for long-term retention.

#. Log in to the DWS console.

#. In the navigation pane on the left, choose **Dedicated Clusters** > **Management** > **Snapshots**. All snapshots are displayed by default.

#. Locate an automated snapshot, choose **More** > **Copy** in the **Operation** column.
...
#. Click **Submit**. The system starts to copy the snapshot for the cluster.
...
To help you restore a cluster deleted by mistake, DWS provides the following policies (supported only in clusters of version 8.2.0 and later):
...
.. |image3| image:: /_static/images/en-us_image_0000002516718671.png
.. |image4| image:: /_static/images/en-us_image_0000002363767369.png
